...
def make_geoms(freqlog, num_geoms, temp, header, bottom):
    ...
    _, atomos, A = lx.tools.sample_geometries(freqlog,num_geoms,temp,3000,warning=False)
...
def classify_only():
    ...
    for file in files:
        ...
        conformations.append(Conformation(rot,scf,lx.tools.fingerprint(file,'.'),num))
    conformations = internal_comparison(conformations)
...
def main():
    ...
    for i, conformation in enumerate(conformations):
        ...
        lx.tools.write_input(atomos,geom,header,'',f'Conformers/Geometry-{i+1}-.com')
