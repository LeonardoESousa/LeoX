...
def interface():
    ...
    if op == '1':
        ...
        if change.lower() == 'n':
            base  = lx.tools.default(base,f"Functional/basis is {base}. If ok, Enter. Otherwise, type functional/basis.\n")
            scrf  = lx.tools.default(scrf,f"SCRF keyword is {scrf}. If ok, Enter. Otherwise, type the desired one.\n")
            cm    = lx.tools.default(cm,f'Charge and multiplicity is {cm}. If ok, Enter. Otherwise, type charge and multiplicity Ex.: 0 1\n')
            nproc = lx.tools.default(nproc,f'nproc is {nproc}. If ok, Enter. Otherwise, type it.\n')
            mem   = lx.tools.default(mem,f"mem is {mem}. If ok, Enter. Otherwise, type it.\n")
        ...
        if question.lower() == 'y':
            try:
                ...
                lx.parser.fatal_error('Something went wrong. Your folder may not contain Geomtetry log files. Goodbye!')
        else:
