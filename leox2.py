...
def pega_modosHP(G, freqlog):
	#massas = pega_massas(freqlog,G) testando conflito
...
def sample_geom(freqlog, base, num_geoms, num_ex, T, nproc, mem, distrib):
	F, M = pega_freq(freqlog)
...
def vibronic_shift(G1, G, NNC, F, M, massas):
	shift = rotaciona(G1,G, massas)
	shift = shift.flatten()
	shift = np.expand_dims(shift,axis=1)
	vibronic, broadening = 0, 0
	NCoords = np.matmul(NNC.T,shift)
	Delta2 = (NCoords**2)*(F*M/hbar2)*(10**(-20))
	print(np.sum(Delta2),'\n')
	vibronic = np.sum(Delta2*F)
	broadening = np.sum(Delta2*(F**2))
	vibronic = (hbar/2)*vibronic
	broadening = (hbar)*np.sqrt(broadening) 
	return vibronic, broadening


#def vibronic_shift(G1, G, NNC, F, M, GO):
#	#print(G1,'\n')
#	#print(G)
#	#input('pausa')
#	shift, atoms = rotaciona(G1,G,GO)
#	vibronic, broadening, delta, delta2 = 0, 0, 0, 0
#	modulo = np.inner(shift.flatten(),shift.flatten())
#	#for i in range(0,len(F)):
#	#	normal = NNC[:,3*i:3*i+3]
#	#	Delta = np.inner(shift.flatten(),normal.flatten())
#	#	delta += Delta**2
#	for i in range(0,len(F)):
#		#Delta = np.sum(NNC[:,3*i:3*i+3]*NNC[:,3*(i+1):3*(i+1)+3])#/np.sqrt(np.sum(NNC[:,3*i:3*i+3]**2))
#		normal = NNC[:,3*i:3*i+3]
#		Delta = np.inner(shift.flatten(),normal.flatten())
#		#Delta = np.sum(shift*NNC[:,3*i:3*i+3])/np.sqrt(np.sum(NNC[:,3*i:3*i+3]**2))
#		Delta2 = (Delta**2)*((10**(-20))*(M[i]*F[i]/hbar2))#*(modulo/delta) 
#		delta2 += (Delta**2)#*(modulo/delta)
#		if delta2 >= modulo:
#			print(i+1)
#			Delta2 = 0
#			#print(Delta, i+1)
...
#		else:
#			vibronic += (Delta2)*F[i]
#			broadening += Delta2*(F[i]**2)
#	print(modulo, delta2)
#	vibronic = (hbar/2)*vibronic
#	broadening = (hbar/np.sqrt(2))*np.sqrt(broadening) 
#	return vibronic, broadening
			...
			with open(file, 'r') as g:
				for line in g: 
					if "Excited State" in line:
						line = line.split()
						f.write("Excited State "+line[2]+"\t"+line[4]+"\t"+line[8][2:]+"\t"+str(vibronic)+"\t"+str(broadening)+"\n")
				f.write("\n")	
...
if op == '1':
	...
	distrib = input("Quer usar a distribuição sem temperatura (Wigner)? s ou n?\n")
	if distrib == 's':
		T = -10.0
	else:	
	...
	if T == 0:
		...
		sys.exit()
	sample_geom(freqlog, base, num_geoms, num_ex, T,nproc, mem, distrib)	
elif op == '3':
	opc = input("Qual o desvio padrão das gaussianas? 0.025 é o padrão. Digite 0 para o automático.\n")
	try:
		opc = float(opc)
	except: 
		...
		sys.exit()
	if opc == 0:
		freqlog = busca_log("É esse o log com a geometria final?")
